...
from visualization.parser.elements.junction import \
    Connection as JunctionConnection
from visualization.parser.elements.junction import Junction
from visualization.parser.elements.junction import \
    LaneLink as JunctionConnectionLaneLink
from visualization.parser.elements.opendrive import OpenDrive
from visualization.parser.elements.road import Road
from visualization.parser.elements.roadElevationProfile import \
    ElevationRecord as RoadElevationProfile
from visualization.parser.elements.roadLanes import Lane as RoadLaneSectionLane
from visualization.parser.elements.roadLanes import \
...
from visualization.parser.elements.roadLateralProfile import \
...
from visualization.parser.elements.roadLink import Neighbor as RoadLinkNeighbor
from visualization.parser.elements.roadLink import \
    Predecessor as RoadLinkPredecessor
from visualization.parser.elements.roadLink import \
    Successor as RoadLinkSuccessor
from visualization.parser.elements.roadtype import RoadType
from visualization.parser.elements.roadtype import Speed as RoadTypeSpeed
...
def parse_opendrive_road_type(road, opendrive_xml_road_type: etree.ElementTree):  # type: ignore
...
def parse_opendrive_road_lane_section(newRoad, lane_section_id, lane_section):
    ...
    newLaneSection.sPos = float(lane_section.get("s"))  # type: ignore
...
def parse_opendrive_road(opendrive, road):
    ...
    newRoad.length = float(road.get("length"))  # type: ignore
...
def calculate_lane_section_lengths(newRoad):
    ...
    for lane_section in newRoad.lanes.lane_sections:
        ...
        # All but the last lane section end at the succeeding one
            ...
            lane_section.length = (
                newRoad.lanes.lane_sections[lane_section.idx + 1].sPos - lane_section.sPos
            )
