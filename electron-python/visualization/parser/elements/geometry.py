...
import numpy as np
from visualization.parser.elements.eulerspiral import EulerSpiral
...
class Geometry(abc.ABC):
    ...
    def start_position(self) -> float:
        ...
        return self._start_position  # type: ignore
...
class Spiral(Geometry):
    ...
    def calc_position(self, s_pos):
        ...
        (x, y, t) = self._spiral.calc(
            s_pos,
            self.start_position[0],  # type: ignore
            self.start_position[1],  # type: ignore
            self._curvStart,
            self.heading,  # type: ignore
        )
