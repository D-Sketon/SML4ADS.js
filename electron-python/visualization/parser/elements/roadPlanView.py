...
import numpy as np
from visualization.parser.elements.geometry import (Arc, Geometry, Line,
                                                    ParamPoly3, Poly3, Spiral)
...
class PlanView:
    ...
    def __init__(self):
        self._geometries = []
        self._precalculation: Optional[np.ndarray] = None
    ...
    def interpolate_cached_values(self, s_pos: float) -> Tuple[np.ndarray, float]:
        ...
        idx = np.abs(cast(np.ndarray, self._precalculation)[:, 0] - s_pos).argmin()
        if s_pos - cast(np.ndarray, self._precalculation)[idx, 0] < 0 or idx + 1 == len(
            cast(np.ndarray, self._precalculation)
        ):
            idx -= 1
        result_pos_x = np.interp(
            s_pos,
            cast(np.ndarray, self._precalculation)[idx: idx + 2, 0],
            cast(np.ndarray, self._precalculation)[idx: idx + 2, 1],
        )
        result_pos_y = np.interp(
            s_pos,
            cast(np.ndarray, self._precalculation)[idx: idx + 2, 0],
            cast(np.ndarray, self._precalculation)[idx: idx + 2, 2],
        )
    ...
    def interpolate_angle(self, idx: int, s_pos: float) -> float:
        ...
        """
        angle_prev = cast(np.ndarray, self._precalculation)[idx, 3]
        angle_next = cast(np.ndarray, self._precalculation)[idx + 1, 3]
        pos_prev = cast(np.ndarray, self._precalculation)[idx, 0]
        pos_next = cast(np.ndarray, self._precalculation)[idx + 1, 0]
