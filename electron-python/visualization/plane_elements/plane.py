...
from visualization.plane_elements.border import Border
...
class ParametricLaneBorderGroup:
    ...
    def calc_border_position(
        ...
        return cast(Border, select_border).calc(
            cast(float, select_offset) + s_pos, width_offset=width_offset, is_last_pos=is_last_pos
        )
    ...
    def get_width_coefficients(self) -> list:
        ...
        return cast(Border, self.outer_border).get_next_width_coeffs(cast(float, self.outer_border_offset))
...
class ParametricLane:
    ...
    def calc_width(self, s_pos: float) -> float:
        ...
        return np.linalg.norm(innerCoords[0] - outerCoords[0])  # type: ignore
    ...
    def to_lanelet_with_mirroring(
        ...
        for i, pos in enumerate(poses):
            ...
            original_width = np.linalg.norm(inner_pos - outer_pos)  # type: ignore
            ...
            if (
                ...
                if mirror_border == "left":
                    ...
                    modified_width = np.linalg.norm(new_outer_pos - inner_pos)  # type: ignore
                    ...
                    modified_width = np.linalg.norm(new_inner_pos - outer_pos)  # type: ignore
    ...
    def zero_width_change_positions(self) -> float:
        ...
        if width_coefficients[0] > 0.0 and all(
            ...
            return [0, self.length]  # type: ignore
    ...
    def maximum_width(self, reverse: bool = False) -> Tuple[float, float]:
        ...
        restricted_roots = roots[
            (np.isreal(roots)) & (roots >= 0) & (roots <= self.length)
        ]
        ...
        return tuple(pos_and_val[max_idx])  # type: ignore
