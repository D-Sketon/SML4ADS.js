...
class Border:
    ...
    def _get_width_index(self, s_pos: float, is_last_pos: bool) -> int:
        ...
        return next(
            (
                ...
                if (
                    (n <= s_pos and (not is_last_pos or s_pos == 0)) or (n < s_pos and is_last_pos)
                )
            ),
    ...
    def calc(self, s_pos: float, width_offset: float = 0.0, is_last_pos: bool = False):
        ...
        try:
            ref_coord, tang_angle = self.reference.calc(  # type: ignore
                self.ref_offset + s_pos, is_last_pos=is_last_pos
            )
        except TypeError:
            ref_coord, tang_angle = self.reference.calc(self.ref_offset + s_pos)  # type: ignore
