...
class ConversionLaneletNetwork(LaneletNetwork):
    ...
    def remove_lanelet(self, lanelet_id: str, remove_references: bool = False):
        ...
        """
        del self._lanelets[lanelet_id]  # type: ignore
        if remove_references:
            for lanelet in self.lanelets:
                ...
                if lanelet.adj_right == lanelet_id:
                    lanelet.adj_right = None  # type: ignore
                if lanelet.adj_left == lanelet_id:
                    lanelet.adj_left = None  # type: ignore
    ...
    def find_lanelet_by_id(self, lanelet_id) -> ConversionLanelet:
        ...
        """
        return self._lanelets.get(lanelet_id)  # type: ignore

    def find_lanelet_by_description(self, description) -> Optional[ConversionLanelet]:
        for lanelet in self._lanelets.values():
            if cast(ConversionLanelet, lanelet).description == description:
                return lanelet  # type: ignore
        return None
    ...
    def _delete_zero_width_parametric_lanes(self):
        ...
        """
        for lanelet in self.lanelets:
            if cast(ConversionLanelet, lanelet).has_zero_width_everywhere():
    ...
    def update_lanelet_id_references(self, old_id: str, new_id: str):
        ...
        for lanelet in self.lanelets:
            ...
            if lanelet.adj_right == old_id:
                lanelet.adj_right = int(new_id)

            if lanelet.adj_left == old_id:
                lanelet.adj_left = int(new_id)
    ...
    def join_and_split_possible_lanes(self):
        ...
        for lanelet in self.lanelets:
            ...
            if lanelet_join or lanelet_split:
                js_targets.append(
                    _JoinSplitTarget(self, cast(ConversionLanelet, lanelet), lanelet_split, lanelet_join)
                )
    ...
    def add_successors_to_lanelet(
        ...
        for successor_id in successor_ids:
            ...
            successor = self.find_lanelet_by_id(successor_id)  # type: ignore
    ...
    def add_predecessors_to_lanelet(
        ...
        for predecessor_id in predecessor_ids:
            ...
            predecessor = self.find_lanelet_by_id(predecessor_id)  # type: ignore
    ...
    def adj_right_consistent_nb(self, lanelet: ConversionLanelet) -> bool:
        ...
        if adj_right:
            if lanelet.adj_right_same_direction:
                ...
                if not lanelet.has_unique_pred_succ_relation(-1, adj_right):  # type: ignore
                    return False
...
class _JoinSplitTarget:
    ...
    def _find_lanelet_by_id(self, lanelet_id: str) -> ConversionLanelet:
        ...
        """
        return self.lanelet_network.find_lanelet_by_id(lanelet_id)  # type: ignore
    ...
    def _move_borders_if_split_and_join(self):
        ...
        lanelet_split = self._js_pairs[0].move_border(
            width=[start_width_split, self.change_width[0]],  # type: ignore
        ...
        self._js_pairs[1].move_border(
            width=[self.change_width[1], start_width_join],  # type: ignore
            linking_side=self.linking_side,
        )
    ...
    def determine_apt_js_pairs(self):
        ...
        while True:
            ...
            if algo_has_finished or self.use_only_single_lanelet():
                break
            if (
                self.split and self.lanelet_network.successor_is_neighbor_of_neighbors_successor(
                    lanelet
                )
    ...
    def _add_join_split_pair(
        ...
        if self.change_width is not None and change_width < self.change_width:  # type: ignore
    ...
    def _determine_main_adjacent_lanelet(self) -> Optional[ConversionLanelet]:
