...
class BehaviorTree:
    ...
    def __init__(self):
        ...
        self.current: Optional[Union[BehaviorNode, BranchNode, CommonTransition, ProbabilityTransition]] = None  # 遍历记录，当前遍历到的元素引用
    ...
    def traver_tree(self, _args, is_force):
        ...
        if len(cast(Union[BranchNode, BehaviorNode], self.current).transitions) == 0:
            self.is_end = True
            node = BehaviorNode()
            node.state = VehicleState.KEEP
            node.duration = 5
            node.target_vel = cast(BehaviorNode, self.current).target_vel
            return node
        if not is_force and isinstance(self.current, BehaviorNode):
            ...
            transition = random.choice(cast(Union[BehaviorNode, BranchNode], self.current).transitions)
            self.current_id = transition[0]
            self.current = self.elements[self.current_id]
            while isinstance(self.current, BranchNode):
                self.__do_probability_trans()
    ...
    def get_root_node(self) -> BehaviorNode:
