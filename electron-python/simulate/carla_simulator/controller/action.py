...
from simulate.carla_simulator.controller.enums import RoadOption, VehicleState
...
class Action:
    ...
    def __init__(self, carla_map: carla.Map, parser, step=5):  # type: ignore
        """
        构造方法
        ...
        assert isinstance(carla_map, carla.Map)  # type: ignore
        self.map = carla_map
        self.step = step
        self.parser = parser
    ...
    def compute_next_wpt_vel(self, vehicle, state, is_junction, prev_road_id=0, prev_wp=None):
        ...
        next_wp = None
        target_wp = None
        if not isinstance(vehicle, carla.Vehicle) or not isinstance(state, VehicleState):  # type: ignore
        ...
        if state == VehicleState.CHANGELEFT:
            ...
            vel = get_speed(vehicle)
            next_wp = prev_wp.next(self.step)[0]  # type: ignore
            return next_wp, vel, target_wp
    ...
    def __change_lane(self, prev_wp, direction=None) -> carla.Waypoint:  # type: ignore
        """
        汽车便道，计算下一个路径点
        ...
        if left_wp is not None and left_wp.lane_type == carla.LaneType.Driving and left_wp.lane_id * prev_wp.lane_id > 0:  # type: ignore
            left_wps = list(left_wp.next(20))
            if left_wps:
                left_change_wp = left_wps[0]
        elif right_wp is not None and right_wp.lane_type == carla.LaneType.Driving and right_wp.lane_id * prev_wp.lane_id > 0:  # type: ignore
            ...
            for ll in conn.lane_links:
                for lane in conn_road.road_lanes.lane_section.lanes:
                    if ll.to == lane.lane_id:
                        ...
                        if wp is not None and wp.lane_type == carla.LaneType.Driving:  # type: ignore
                            flag = True
    ...
    def __turn_follow_connecting(self, vehicle: carla.Vehicle, prev_wp: carla.Waypoint, prev_road_id: int):  # type: ignore
        """
