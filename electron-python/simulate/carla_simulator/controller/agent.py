...
class Agent:
    ...
    def __init__(self, carla_map, parser, vehicle, behavior_tree: BehaviorTree, action: Action, args_lateral=None,
        ...
        self.start_time: Optional[datetime] = None  # 当前行为的开始时间
        self.clock = 0.0
        ...
        self.behavior_node: BehaviorNode = behavior_tree.get_root_node()  # 当前行为节点
    ...
    def run(self, args):
        ...
        if state == VehicleState.IDLE:
            self.vehicle.enable_constant_velocity(carla.Vector3D(0, 0, 0))  # type: ignore
            return
        ...
        if self.behavior_node.acc != 0 and abs(get_speed(self.vehicle) / 3.6 - self.behavior_node.target_vel) > 2:
            ...
            print(f'control[throttle:{control.throttle};brake:{control.brake};steer:{control.steer}]')
            self.vehicle.add_force(carla.Vector3D(0, 0, 0))  # type: ignore
            self.vehicle.apply_control(control)
    ...
    def __compute_wp_vel(self, state, is_junction):
        ...
        next_wp, vel, target_wp = self.action.compute_next_wpt_vel(self.vehicle, state, is_junction=is_junction,
                                                                   prev_road_id=self.prev_road_id, prev_wp=self.prev_wp)  # type: ignore
        if next_wp is not None:
            ...
            self.next_wp, vel, self.turn_target_wp = self.action.compute_next_wpt_vel(self.vehicle,
                                                                                      ...
                                                                                      prev_road_id=self.prev_road_id)  # type: ignore
    ...
    def update_state(self, wp, args=None):
        ...
        if self.model == 'sy':
            ...
            time_interval = current_time.timestamp() - cast(datetime, self.start_time).timestamp()
        ...
        if self.behavior_node.state == VehicleState.TURNLEFT or \
            ...
            if self.behavior_node.duration != -1 and time_interval >= self.behavior_node.duration:
                ...
                self.behavior_node = self.behavior_tree.traver_tree(is_force=True, _args=None)  # type: ignore
                self.__set_min_duration()
                print(f'duration:{time_interval}, model:{self.model}')
                print(args)
    ...
    def add_args(self, args, current_wp):
        ...
        args['has_left_lane'] = True if lane_change == carla.LaneChange.Left or lane_change == carla.LaneChange.Both else False  # type: ignore
