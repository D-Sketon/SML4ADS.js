...
class MapParser:
    ...
    def __parse_lanes(self, road_id, lanes):
        ...
        results = []
        for la in lanes:
            if la.get('type') == 'driving':
                lane = Lane(road_id, int(la.get('id')), la.get('type'), la.get('level'))
                link = la.find('link')
                width = la.find('width')
                if link is not None:
                    pre = link.find('predecessor')
    ...
    def __parse_junction_connection(self, connection):
        ...
        lane_links = []
        for lk in connection.iter('laneLink'):
            lane_links.append(JunctionLaneLink(int(lk.get('from')), int(lk.get('to'))))
